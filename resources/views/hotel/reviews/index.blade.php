...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-dark-100 p-6 rounded-xl shadow-md border border-gray-200 dark:border-dark-200 mb-6 transition-colors duration-300">
        <div>
            <h1 class="text-3xl font-bold text-gray-800 dark:text-dark-700">Customer Reviews</h1>
            <p class="text-gray-600 dark:text-dark-500">View and manage customer feedback and ratings</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white dark:bg-dark-100 rounded-xl shadow-lg p-6 border dark:border-dark-200 transition-colors duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-dark-400">Total Reviews</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-dark-700">{{ number_format($stats['total']) }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-dark-100 rounded-xl shadow-lg p-6 border dark:border-dark-200 transition-colors duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-dark-400">Average Rating</p>
                    <div class="flex items-center gap-2">
                        <p class="text-2xl font-bold text-amber-600 dark:text-amber-400">{{ $stats['average_rating'] }}</p>
                ...
                <div class="bg-amber-100 dark:bg-amber-900 p-3 rounded-full">
                    <i class="fas fa-star-half-alt text-amber-600 dark:text-amber-400 text-xl"></i>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-100 rounded-xl shadow-lg p-6 border dark:border-dark-200 transition-colors duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-dark-400">Recent (30 days)</p>
                    <p class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ number_format($stats['recent_reviews']) }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-dark-100 rounded-xl shadow-lg p-6 border dark:border-dark-200 transition-colors duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-dark-400">Highest Rating</p>
    ...
    <div class="bg-white dark:bg-dark-100 rounded-xl shadow-lg p-6 border dark:border-dark-200 transition-colors duration-300">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-700 mb-4">Rating Distribution</h3>
        <div class="space-y-3">
            @for($rating = 5; $rating >= 1; $rating--)
                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-1 w-16">
                        <span class="text-sm font-medium dark:text-dark-700">{{ $rating }}</span>
                        <i class="fas fa-star text-amber-400 text-xs"></i>
                    </div>
                    <div class="flex-1 bg-gray-200 dark:bg-dark-200 rounded-full h-2">
                        <div class="bg-amber-500 dark:bg-amber-400 h-2 rounded-full transition-all duration-300" style="width: {{ $percentage }}%"></div>
                    </div>
                    <span class="text-sm text-gray-600 dark:text-dark-500 w-12">{{ $count }}</span>
                    <span class="text-sm text-gray-500 dark:text-dark-400 w-12">{{ number_format($percentage, 1) }}%</span>
                </div>
            @endfor
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-dark-100 rounded-xl shadow-lg p-6 border dark:border-dark-200 transition-colors duration-300">
        <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                <!-- Rating Filter -->
                <div class="relative">
                    <select id="ratingFilter" class="appearance-none bg-gray-100 dark:bg-dark-200 border border-gray-300 dark:border-dark-200 rounded-lg px-3 py-2 pr-8 text-sm text-gray-900 dark:text-dark-700 transition-colors duration-300">
                    ...
                    <i class="fas fa-chevron-down absolute right-2 top-2.5 text-gray-400 dark:text-dark-400 text-xs"></i>
                </div>
            </div>
            ...
            <div class="relative">
                <form method="GET" class="flex">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Search reviews..."
                           class="pl-9 pr-3 py-2 w-64 rounded-lg border border-gray-300 dark:border-dark-200 focus:outline-none focus:ring-1 focus:ring-gray-400 dark:focus:ring-blue-500 bg-white dark:bg-dark-100 text-gray-900 dark:text-dark-700 transition-colors duration-300"
                    />
    ...
    <!-- Reviews List -->
    <div class="bg-white dark:bg-dark-100 rounded-xl shadow-lg border dark:border-dark-200 transition-colors duration-300">
        <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-700 mb-4">Reviews ({{ $reviews->total() }})</h3>
            
            @forelse($reviews as $review)
                <div class="border-b border-gray-200 dark:border-dark-200 last:border-b-0 py-6 first:pt-0 last:pb-0">
                    <div class="flex flex-col md:flex-row md:items-start gap-4">
                        ...
                        <div class="flex items-center gap-3 md:w-48 flex-shrink-0">
                            ...
                            <div>
                                <p class="font-medium text-gray-900 dark:text-dark-700">{{ $review->tourist->name ?? 'Anonymous' }}</p>
                                <p class="text-sm text-gray-500 dark:text-dark-400">{{ $review->created_at->format('M d, Y') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-3 mb-2">
                                ...
                                <h4 class="font-semibold text-gray-900 dark:text-dark-700">{{ $review->title }}</h4>
                            </div>
                            
                            <!-- Review Description -->
                            <p class="text-gray-700 dark:text-dark-500 mb-3">{{ $review->description }}</p>
