...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-md border border-gray-200 mb-6">
        <div>
            ...
            <p class="text-gray-600">Manage tourist activities and experiences across locations</p>
        </div>
        ...
        <div class="flex flex-col md:flex-row md:items-center gap-4">
            ...
            <div class="flex items-center gap-3">
                <form method="GET" class="relative">
                    ...
                    @endif
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Search activities..."
                ...
                <button id="add-activity-btn" class="px-4 py-2.5 bg-gray-900 hover:bg-gray-800 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors duration-200 flex items-center gap-2">
            ...
            <tbody class="text-gray-800 text-sm">
                    @forelse($activities as $activity)
                        <tr class="border-t hover:bg-gray-50 transition duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4">
                                @if($activity->image || $activity->main_image)
                                    <img src="{{ $activity->image_url }}" class="w-24 h-16 object-cover rounded border">
                                @else
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 rounded-full text-xs font-semibold
                                  {{ strtolower($activity->status)==='active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
...
@push('scripts')
    <!-- Custom scripts for activities page -->
    <script>
        // Map: activity_id -> image URL or null
        const ACTIVITY_IMAGE = @json($ACTIVITY_IMAGE);
    ...
    <script>
        ...
        document.querySelectorAll('[id^="edit-activity-btn-"]').forEach(button => {
            button.addEventListener('click', function () {
                ...
                if (ACTIVITY_IMAGE[activity.id]) {
                    currentImg.src = ACTIVITY_IMAGE[activity.id];
                    currentWrap.classList.remove('hidden');
                } else {
                    currentImg.src = '';
                    currentWrap.classList.add('hidden');
        ...
        deleteCurrentButton.addEventListener('click', function () {
            const actId = document.getElementById('activity-form').action.split('/').pop();
            if (!actId || !ACT_IMAGE_DELETE_URL) return;

            if (confirm('Delete this image?')) {
                fetch(ACT_IMAGE_DELETE_URL.replace(':act', actId), {
                    method: 'DELETE',
                    headers: { 'X-CSRF-TOKEN': CSRF_TOKEN, 'Accept': 'application/json' },
                }).then(r => r.json()).then(data => {
                    if (data.success) {
                        ACTIVITY_IMAGE[actId] = null; // update map
                        currentImg.src = '';
                        currentWrap.classList.add('hidden');
                        if (window.pushToast) window.pushToast('Image deleted successfully');
                    } else {
                        if (window.pushToast) window.pushToast(data.message || 'Failed to delete image', 'error');
                    }
                }).catch(() => {
                    if (window.pushToast) window.pushToast('Network error', 'error');
                });
            }
        });
        ...
        document.querySelectorAll('.delete-activity-form').forEach(form => {
            form.addEventListener('submit', function (e) {
                e.preventDefault();
                deleteForm = this;
                const name = this.querySelector('input[name="activity_name"]')?.value || 'this activity';
